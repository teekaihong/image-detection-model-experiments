...
class VariationalEncoder(nn.Module):
    def __init__(self, dropout=0., latent_dims:int=latent_dims):
        ...
        self.down1 = DownsampleBlock(3, 8, 1, dropout=dropout)
        self.down2 = DownsampleBlock(8, 16, 2, dropout=dropout)
        self.down3 = DownsampleBlock(16, 32, 4, dropout=dropout)
        self.mean = ResidualBlock(32, self.latent_channels, 2, dropout=dropout)
...
class VariationalDecoder(nn.Module):
    def __init__(self, dropout=0., latent_dims:int=latent_dims):
        super().__init__()
        self.latent_channels = latent_dims
        self.res = ResidualBlock(self.latent_channels, 1024, 64, dropout=dropout)
        self.up1 = UpsampleBlock(1024, 768, 48, dropout=dropout)
        self.res1 = ResidualBlock(768, 768, 48, dropout=dropout)
        ...
        self.out = ResidualBlock(256, 3, 1, dropout=dropout)
...
class DetectorHead(nn.Module):
    def __init__(self, num_classes:int, latent_channels:int) -> None:
        ...
        super().__init__()
        self.down1 = DownsampleBlock(latent_channels, latent_channels, 2)
        self.flatten = nn.Flatten()
        self.linear = nn.Linear(latent_channels*12*15, num_classes + 4)
